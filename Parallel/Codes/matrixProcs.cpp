...
// #define FASTER
    ...
    void multiply (float *A, float *B, float *out, int size) {
        int shmMal = shmget(12345, size * size * sizeof(float), IPC_CREAT | 0666);
            if (shmMal < 0) return;
        float *output = (float *) shmat(shmMal, NULL, 0);
            if (output == (float *) -1) return;
        ...
        for (int tid=0;tid<8;tid++) {
            ...
            if (pid == 0) {
                float *temp = new float[size];
                const int start = tid * size / NUM_THREADS;
                const int stop  = tid + 1 == NUM_THREADS ? size : (tid + 1) * size / NUM_THREADS;
        ...
        std::copy(output, output+size, out);
        shmdt((void *) output);
